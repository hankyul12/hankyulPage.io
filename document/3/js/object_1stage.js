    ...
    let user=urlParams.get('user_name');
    ...
    const moon = {
        x: canvas.width-(canvas.width*0.2),
        y: -190,
        width: 900,      
        height: 900,
        speed : 0.2,
    ...
    function gameLoop() { //게임시작
        ...
        if(boss.y<(canvas.height)*0.1){
            ...
            setTimeout(function(){
                ...
                if(sec_timer%7===1){
                    boss.img='../image/boss_teleportation.png';
                    setTimeout(function(){
                        boss.x=ran;
                        setTimeout(function () { 
                            boss.img='../image/spaceship.png';
                         })
                    },100);
                }
            }, 2000);
     ...
     function moveBombs() {   //폭탄 움직이기
        bombs.forEach(bomb => {
            ...
            if(isCollision(bomb,boss)){ 
                boss.hp -= bomb.atk;
                boss.img="../image/spaceship_bomb.png";
                bombs.splice(bombs.indexOf(bomb), 1);
                setTimeout(function(){
                    boss.img="../image/spaceship.png";
                },400);
    ...
    function moveBullets() {    //총움직이기, 적없어지는거랑 똑같이 구현 
        bullets.forEach(bullet => {
            ...
            if(isCollision(bullet,boss)){ //총알에 보스 충돌
                boss.hp -= player.atk;
                boss.img="../image/spaceship_attack.png";
                bullets.splice(bullets.indexOf(bullet), 1);
                setTimeout(function(){
                    boss.img="../image/spaceship.png";
                },400);
    ...
    function showGameOver() { //게임오버시 안보였던거 block처리
        gameOver = true;
        let gold_calc1 =Math.floor(( (500-boss.hp) / 500 ) * 1000); //깐 체력비례 코인획득
        // 까인 체력 퍼센트 *10만큼 골드획득
    ...
    function restartGame() { //리겜, 설정초기화
        ...
        boss.hp=500-(boss_hp_i*10); // 적HP감소
        boss.y= -100;
